...
CONCEPT="Hash Tables"
UNOPT_DS="Linear scan over token list"
OPT_DS="Hash table via std::unordered_set"
UNOPT_HOW="Each token check compares against many entries until match or end"
OPT_HOW="Hashing maps tokens to buckets, reducing repeated comparisons"
UNOPT_COMPLEXITY="Lookup cost grows linearly with active token count"
OPT_COMPLEXITY="Average-case lookup stays near constant time"
...
printf '\nCompiling...\n'
g++ -std=c++20 -O2 "$UNOPT_SRC" -o "$BUILD_DIR/unoptimized_demo"
g++ -std=c++20 -O2 "$OPT_SRC" -o "$BUILD_DIR/optimized_demo"
